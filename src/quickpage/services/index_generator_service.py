...
class IndexGeneratorService:
    ...
    def group_neuron_types_by_roi(
        self, index_data: List[Dict[str, Any]], roi_hierarchy_service
    ) -> List[Dict[str, Any]]:
        ...
        for entry in index_data:
            ...
            if parent_roi != "Other":
                parent_roi = (
                    roi_hierarchy_service._clean_roi_name(parent_roi) or "Other"
                )
                ...
                sorted_groups.append(
                    {
                        ...
                        "neuron_types": sorted(
                            grouped_data[parent_roi], key=lambda x: x["name"]
                        ),
                    }
                )
            ...
            sorted_groups.append(
                {
                    "parent_roi": "Other",
                    "neuron_types": sorted(
                        grouped_data["Other"], key=lambda x: x["name"]
                    ),
                }
            )
