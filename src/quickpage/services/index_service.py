...
class IndexService:
    ...
    def _generate_index_data(self, neuron_types):
        ...
        if missing_cache_count > 0:
            logger.warning(
                f"Index data generation completed: {len(index_data)} entries, {cached_count} with cache, {missing_cache_count} missing cache. Run 'quickpage generate' to populate cache."
            )
        else:
            logger.info(
