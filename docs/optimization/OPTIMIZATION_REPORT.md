...
Performance analysis of `quickpage create-index --include-roi-analysis` revealed severe bottlenecks in database query patterns and ROI analysis. The current implementation takes **50+ minutes** to process a typical dataset, with the majority of time spent on inefficient database queries to the NeuPrint server.
...
time quickpage -c config.cns.yaml create-index --include-roi-analysis

# After each optimization phase
time quickpage -c config.cns.yaml create-index --include-roi-analysis --optimized
