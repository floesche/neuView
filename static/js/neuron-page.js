$(document).ready(function() {
    ...
    // This will be populated by template data injection
    if (typeof window.templateData !== 'undefined' && window.templateData.roiData) {
        window.templateData.roiData.forEach(function(roi) {
    ...
    if (typeof window.templateData !== 'undefined' && window.templateData.upstreamData) {
        window.templateData.upstreamData.forEach(function(partner) {
            var key = partner.type + ' (' + (partner.soma_side || '') + ')';
            upstreamPreciseData[key] = partner.percentage || 0;
        });
    }

    // Create lookup map for precise downstream connection percentages
    var downstreamPreciseData = {};
    if (typeof window.templateData !== 'undefined' && window.templateData.downstreamData) {
        window.templateData.downstreamData.forEach(function(partner) {
            var key = partner.type + ' (' + (partner.soma_side || '') + ')';
            downstreamPreciseData[key] = partner.percentage || 0;
        });
    }
    ...
    function createConnectionsFilter(tableId, connectionsColumnIndex) {
        return function(settings, data, dataIndex) {
            ...
            var minConnections = Math.pow(10, logValue);
            var actualConnections = parseFloat(data[connectionsColumnIndex]) || 0;

            return actualConnections >= minConnections;
        };
    }
    ...
    // Initialize ROI table if it exists
    if ($('#roi-table').length) {
        ...
        var roiTable = $('#roi-table').DataTable({
            ...
            "drawCallback": function(settings) {
                calculateCumulativePercentages(this.api(), 2, 3); // % Input and cumulative
                calculateCumulativePercentages(this.api(), 5, 6); // % Output and cumulative
            },
    ...
    // Initialize upstream table if it exists
    if ($('#upstream-table').length) {
        ...
        var upstreamTable = $('#upstream-table').DataTable({
            ...
            "initComplete": function(settings, json) {
                // Create slider in header after table initialization
                var neuronType = window.templateData ? window.templateData.neuronType : 'neuron';
                createConnectionsSliderInHeader('upstream-table', neuronType);
                setupConnectionsSlider('upstream-connections-slider', 'upstream-slider-value', this.api());
            }
        });
    }

    // Initialize downstream table if it exists
    if ($('#downstream-table').length) {
        ...
        var downstreamTable = $('#downstream-table').DataTable({
            ...
            "initComplete": function(settings, json) {
                // Create slider in header after table initialization
                var neuronType = window.templateData ? window.templateData.neuronType : 'neuron';
                createConnectionsSliderInHeader('downstream-table', neuronType);
