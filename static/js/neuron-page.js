...
function calculateDownstreamCumulativePercentages(table, percentageCol, cumulativeCol, downstreamPreciseData) {
    var cumulativeSum = 0;
...
function calculateROICumulativePercentages(table, percentageCol, cumulativeCol, roiPreciseData) {
    var cumulativeSum = 0;
...
function initializeTitleTooltips() {
    ...
    document.querySelectorAll('[title]:not(abbr)').forEach(function(element) {
        ...
        if (tooltipText && tooltipText.trim() && !element._titleTooltipInitialized) {
