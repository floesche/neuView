...
class PopCommandProfiler:
    ...
    def _run_pop_command(self, output_dir: Optional[str] = None, verbose: bool = True) -> Tuple[bool, str, float]:
        """
    ...
    def profile_batch_pop(self, num_operations: int = 10) -> BatchPopMetrics:
        ...
        return BatchPopMetrics(
            ...
            throughput_operations_per_second=total_ops / total_time if total_time > 0 else 0,
    ...
    def analyze_queue_characteristics(self) -> Dict[str, Any]:
            ...
            try:
                ...
                if data and 'options' in data:
                    ...
                    config_key = (
                        ...
                        options.get('include-3d-view', False)
                    )
        ...
        return {
            ...
            'average_file_size_bytes': sum(file_sizes) / len(file_sizes) if file_sizes else 0,
            ...
            'popular_configurations': sorted(configurations.items(), key=lambda x: x[1], reverse=True)[:5]
        }
    ...
    def benchmark_queue_processing_patterns(self) -> Dict[str, Any]:
        ...
        results['sequential'] = {
            ...
            'success_rate': sequential_metrics.successful_operations / sequential_metrics.total_operations,
    ...
    def identify_bottlenecks(self) -> Dict[str, Any]:
        ...
        generation_times = [m.page_generation_time for m in self.metrics_history if m.success]
        ...
        slow_operations = [m for m in self.metrics_history if m.total_time > slow_threshold]
        ...
        return {
            'performance_stats': {
                ...
                'generation_time_percentage': (avg_generation / avg_total * 100) if avg_total > 0 else 0
            },
            'bottlenecks': {
                'slow_operations_count': len(slow_operations),
                'slow_operations_threshold': slow_threshold,
                'primary_bottleneck': 'page_generation' if avg_generation > avg_total * 0.7 else 'queue_management'
            },
            'failure_analysis': {
    ...
    def _analyze_common_errors(self, failed_operations: List[PopPerformanceMetrics]) -> List[str]:
        ...
        for op in failed_operations:
            if op.error_message:
                ...
                if 'timeout' in error:
                    ...
                    error_patterns['file_not_found'] = error_patterns.get('file_not_found', 0) + 1
    ...
    def suggest_optimizations(self, bottlenecks: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    def generate_comprehensive_report(self):
        ...
        print(f"   Total queue size: {queue_analysis['total_queue_size_bytes']/1024:.1f}KB")
        print(f"   Average file size: {queue_analysis['average_file_size_bytes']:.0f} bytes")
        print(f"   Unique neuron types: {queue_analysis['unique_neuron_types']}")
        print(f"   Configuration variations: {queue_analysis['configuration_variations']}")
        ...
        print(f"     Successful operations: {batch_metrics.successful_operations}/{batch_metrics.total_operations}")
        print(f"     Average time per operation: {batch_metrics.average_time_per_operation:.3f}s")
        print(f"     Throughput: {batch_metrics.throughput_operations_per_second:.2f} ops/sec")
        print(f"     Success rate: {batch_metrics.successful_operations/batch_metrics.total_operations*100:.1f}%")
        ...
        if 'error' not in bottlenecks:
            ...
            print(f"   Page generation time: {perf_stats['generation_time_percentage']:.1f}% of total")
            print(f"   Primary bottleneck: {bottlenecks['bottlenecks']['primary_bottleneck']}")
            print(f"   Failure rate: {bottlenecks['failure_analysis']['failure_rate']*100:.1f}%")
        ...
        print(f"   Queue scanning overhead: {patterns['filesystem']['queue_scan_average_time']*1000:.1f}ms")
        ...
        if batch_metrics.throughput_operations_per_second < 0.5:
            print(f"   ⚠️  LOW THROUGHPUT: {batch_metrics.throughput_operations_per_second:.2f} ops/sec")
            print(f"   → Focus on page generation and cache optimizations")
        elif batch_metrics.throughput_operations_per_second < 1.0:
            print(f"   📈 MODERATE THROUGHPUT: {batch_metrics.throughput_operations_per_second:.2f} ops/sec")
            print(f"   → Queue management and I/O optimizations recommended")
        else:
            print(f"   ✅ GOOD THROUGHPUT: {batch_metrics.throughput_operations_per_second:.2f} ops/sec")
