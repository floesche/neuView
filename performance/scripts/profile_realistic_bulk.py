...
class RealisticBulkProfiler:
    ...
    def simulate_current_bulk_generation(
    ...
    def simulate_optimized_bulk_generation(
        self, neuron_types: List[str]
    ) -> Dict[str, Any]:
        """
        ...
        """
        print(
            f"⚡ Simulating OPTIMIZED bulk generation for {len(neuron_types)} types..."
        )
        ...
        for neuron_type, cache_data in neuron_cache_data.items():
            try:
                ...
                soma_sides = self._convert_soma_sides_format(
                    cache_obj.soma_sides_available
                )
                soma_sides_data[neuron_type] = soma_sides
    ...
    def validate_data_consistency(
        self, current_results: Dict, optimized_results: Dict
    ) -> Dict[str, Any]:
        """
        ...
        consistency_rate = (
            len(consistent_types) / len(current_soma) if current_soma else 0
        )
    ...
    def generate_performance_report(
        self, current: Dict, optimized: Dict, io_analysis: Dict, consistency: Dict
    ):
        """
        ...
        print(
            f"   Optimized - Neuron cache read:  {optimized['neuron_cache_time']:.4f}s"
        )
        print(
            f"   Optimized - Soma extraction:    {optimized['soma_extraction_time']:.4f}s"
        )
        ...
        print(
            f"   I/O operations saved:           {current['total_cache_reads'] - optimized['total_cache_reads']}"
        )
        print(
            f"   Storage space saved:            {io_analysis['size_reduction'] / 1024:.1f}KB"
        )
        ...
        print(
            f"   Consistency rate:               {consistency['consistency_rate'] * 100:.1f}%"
        )
        ...
        if consistency["inconsistent_details"]:
            ...
            for item in consistency["inconsistent_details"]:
                print(
                    f"   {item['type']}: current={item['current']}, optimized={item['optimized']}"
                )
        ...
        overhead_percentage = (
            (soma_overhead / total_bulk_time * 100) if total_bulk_time > 0 else 0
        )

        print(f"\n🎯 BULK GENERATION IMPACT:")
        print(
            f"   Soma cache overhead:            {soma_overhead:.4f}s ({overhead_percentage:.1f}% of total)"
        )
        print(
            f"   Files eliminated:               {io_analysis['soma_cache_files_count']}"
        )
        ...
        if consistency["consistency_rate"] > 0.95:  # 95% consistency
            ...
            print(
                f"   - High data consistency ({consistency['consistency_rate'] * 100:.1f}%)"
            )
            ...
            print(
                f"   - Data consistency too low ({consistency['consistency_rate'] * 100:.1f}%)"
            )
...
def main():
    ...
    try:
        ...
        consistency = profiler.validate_data_consistency(
            current_results, optimized_results
        )

        # Generate comprehensive report
        profiler.generate_performance_report(
            current_results, optimized_results, io_analysis, consistency
        )
