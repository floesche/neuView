...
class TestNeuronSelectionService:
    ...
    def test_get_available_soma_sides_single_side_no_combined_link(self, selection_service, mock_connector):
        ...
        mock_result.__getitem__.return_value.tolist.return_value = ["L"]  # Only left side

        # Mock count query result (single left side, no unknowns)
        mock_count_result = MagicMock()
        mock_count_result.empty = False
        mock_count_result.iloc = [{'leftCount': 5, 'rightCount': 0, 'middleCount': 0, 'totalCount': 5}]
    ...
    def test_get_available_soma_sides_multiple_sides_gets_combined_link(self, selection_service, mock_connector):
        """Test that multi-side neuron types get combined navigation links."""
        # Mock query result with multiple soma sides
        mock_result = MagicMock()
        mock_result.empty = False
        mock_result.__getitem__.return_value.tolist.return_value = ["L", "R"]  # Left and right sides

        # Mock count query result (left and right sides)
        mock_count_result = MagicMock()
        mock_count_result.empty = False
        mock_count_result.iloc = [{'leftCount': 8, 'rightCount': 12, 'middleCount': 0, 'totalCount': 20}]
    ...
    def test_get_available_soma_sides_right_only_no_combined_link(self, selection_service, mock_connector):
        """Test that right-only neuron types don't get combined navigation links."""
        # Mock query result with only right soma side
        mock_result = MagicMock()
        mock_result.empty = False
        mock_result.__getitem__.return_value.tolist.return_value = ["R"]  # Only right side

        # Mock count query result (single right side, no unknowns)
        mock_count_result = MagicMock()
        mock_count_result.empty = False
        mock_count_result.iloc = [{'leftCount': 0, 'rightCount': 12, 'middleCount': 0, 'totalCount': 12}]
    ...
    def test_get_available_soma_sides_middle_only_no_combined_link(self, selection_service, mock_connector):
        """Test that middle-only neuron types don't get combined navigation links."""
        # Mock query result with only middle soma side
        mock_result = MagicMock()
        mock_result.empty = False
        mock_result.__getitem__.return_value.tolist.return_value = ["M"]  # Only middle side

        # Mock count query result (single middle side, no unknowns)
        mock_count_result = MagicMock()
        mock_count_result.empty = False
        mock_count_result.iloc = [{'leftCount': 0, 'rightCount': 0, 'middleCount': 8, 'totalCount': 8}]
        ...
        result = selection_service.get_available_soma_sides("MiddleOnly", mock_connector)
    ...
    def test_get_available_soma_sides_with_c_mapping_to_middle(self, selection_service, mock_connector):
        """Test that 'C' soma side maps to middle, not combined."""
        # Mock query result with 'C' soma side (which maps to middle)
        mock_result = MagicMock()
        mock_result.empty = False
        mock_result.__getitem__.return_value.tolist.return_value = ["L", "C"]  # Left and center (maps to middle)

        # Mock count query result - two sides with data should get combined link
        mock_count_result = MagicMock()
        mock_count_result.empty = False
        mock_count_result.iloc = [{'leftCount': 3, 'rightCount': 0, 'middleCount': 4, 'totalCount': 7}]
        ...
        result = selection_service.get_available_soma_sides("WithCenter", mock_connector)
        ...
        expected_combined_filename = FileService.generate_filename("WithCenter", "combined")
    ...
    def test_get_available_soma_sides_three_sides_gets_combined_link(self, selection_service, mock_connector):
        """Test that three-side neuron types get combined navigation links."""
        # Mock query result with all three soma sides
        mock_result = MagicMock()
        mock_result.empty = False
        mock_result.__getitem__.return_value.tolist.return_value = ["L", "R", "M"]  # All sides

        # Mock count query result (all three sides)
        mock_count_result = MagicMock()
        mock_count_result.empty = False
        mock_count_result.iloc = [{'leftCount': 4, 'rightCount': 3, 'middleCount': 2, 'totalCount': 9}]
        ...
        expected_combined_filename = FileService.generate_filename("AllSides", "combined")
    ...
    def test_get_available_soma_sides_empty_result(self, selection_service, mock_connector):
    ...
    def test_get_available_soma_sides_none_result(self, selection_service, mock_connector):
    ...
    def test_get_available_soma_sides_fafb_dataset_single_side(self, selection_service, mock_connector):
        ...
        result = selection_service.get_available_soma_sides("FAFB_Single", mock_connector)
    ...
    def test_get_available_soma_sides_fafb_dataset_multiple_sides(self, selection_service, mock_connector):
        ...
        mock_count_result = MagicMock()
        mock_count_result.empty = False
        mock_count_result.iloc = [{'leftCount': 6, 'rightCount': 8, 'middleCount': 0, 'totalCount': 14}]
        ...
        result = selection_service.get_available_soma_sides("FAFB_Multi", mock_connector)
        ...
        expected_combined_filename = FileService.generate_filename("FAFB_Multi", "combined")
    ...
    def test_get_available_soma_sides_single_side_with_unknown_gets_combined(self, selection_service, mock_connector):
        """Test that single side with unknown neurons gets combined navigation links."""
        # Mock query result with single side available
        mock_result = MagicMock()
        mock_result.empty = False
        mock_result.__getitem__.return_value.tolist.return_value = ["L"]  # Only left side visible

        # Mock count query result (single left side with unknown neurons)
        mock_count_result = MagicMock()
        mock_count_result.empty = False
        mock_count_result.iloc = [{'leftCount': 3, 'rightCount': 0, 'middleCount': 0, 'totalCount': 8}]  # 5 unknown
        ...
        result = selection_service.get_available_soma_sides("MixedWithUnknown", mock_connector)

        # Should have left and combined links (because of unknown neurons)
        expected_left_filename = FileService.generate_filename("MixedWithUnknown", "left")
        expected_combined_filename = FileService.generate_filename("MixedWithUnknown", "combined")
