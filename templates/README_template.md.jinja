# {{ config.html.title_prefix }} - Dataset Documentation
...
This website provides an interactive catalog of neuron types from the {{ config.neuprint.dataset }} dataset. The catalog allows you to explore detailed information about each neuron type, including connectivity patterns, anatomical distributions, and morphological properties.
...
- **neuPrint**: Direct link to the source neuPrint database at {{ config.neuprint.server }}
...
{% if config.output.include_3d_view %}
